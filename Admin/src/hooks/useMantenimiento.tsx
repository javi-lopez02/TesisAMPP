...
export const useMantenimiento = () => {
  ...
  const softDelete = useCallback(async (id: string) => {
    ...
    setError(null);
    try {
      await deleteMantenimientoRequest(id);
      setMantenimientos((prev) => prev?.filter((m) => m.id !== id) ?? null);
      toastSuccess(
      ...
      const messages = handleAxiosError(err);
      setError(messages);
      toastError("Fallo al Borrar", "Verifique permisos");
      return { success: false, error: err };
    } finally {
      setLoading(false);
    }
