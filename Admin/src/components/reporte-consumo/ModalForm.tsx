// src/components/reporte-consumo/ModalForm.tsx
...
const inputClass = (hasError: boolean) =>
  `w-full rounded-lg border px-3.5 py-2.5 text-[13px] text-[#0e1f4d] outline-none transition placeholder:text-gray-300 dark:bg-black/3 dark:text-gray-500 dark:placeholder:text-white/20 ${
    hasError
      ...
      <div className="w-full max-w-md animate-in fade-in zoom-in-95 duration-200 rounded-2xl border border-black/[0.07] bg-white shadow-xl dark:border-white/[0.07] dark:bg-[#0e1a35]">
        ...
        <div className="flex items-center justify-between border-b border-black/[0.07] px-5 py-4 dark:border-white/[0.07]">
          <div className="flex items-center gap-2.5">
            <div className={`flex h-8 w-8 items-center justify-center rounded-lg ${isEditar ? "bg-[#E6F1FB]" : "bg-[#EAF3DE]"}`}>
              {isEditar ? <Pencil size={14} className="text-[#185FA5]" /> : <Plus size={14} className="text-[#3B6D11]" />}
            </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div>
              <label className="mb-1.5 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wide text-gray-400 dark:text-white/40">
                <Car size={11} /> Asignación <span className="text-[#CC1A2E]">*</span>
              </label>
              <select
              ...
              {errors.asignacionId && (
                <p className="mt-1 text-[11px] text-[#CC1A2E]">{errors.asignacionId}</p>
              )}
              {asignacionSeleccionada && (
                <p className="mt-1 text-[10px] text-gray-400 dark:text-white/30">
                  Responsable: {asignacionSeleccionada.responsable?.nombre} {asignacionSeleccionada.responsable?.apellidos}
                </p>
              )}
            </div>
            ...
            <div>
              <label className="mb-1.5 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wide text-gray-400 dark:text-white/40">
                <Droplet size={11} /> Consumo real (L) <span className="text-[#CC1A2E]">*</span>
              </label>
              <div className="relative">
                <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[13px] font-semibold text-gray-400 dark:text-white/30">L</span>
                <input
                  type="number"
                  min="0"
                  step="0.01"
              ...
              {errors.consumoReal ? (
                <p className="mt-1 text-[11px] text-[#CC1A2E]">{errors.consumoReal}</p>
              ) : form.consumoReal && helpers?.validateNumeroFormat(form.consumoReal, 0.01, "Consumo") ? (
                <p className="mt-1 text-[11px] text-[#BA7517]">{helpers.validateNumeroFormat(form.consumoReal, 0.01, "Consumo")}</p>
              ) : (
                <p className="mt-1 text-[11px] text-gray-400 dark:text-white/30">Litros consumidos</p>
              )}
            </div>
            ...
            <div>
              <label className="mb-1.5 flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-wide text-gray-400 dark:text-white/40">
                <Gauge size={11} /> Kilometraje recorrido <span className="text-[#CC1A2E]">*</span>
              </label>
              <input
                type="number"
                min="0"
              ...
              {errors.kilometrajeRecorrido ? (
                <p className="mt-1 text-[11px] text-[#CC1A2E]">{errors.kilometrajeRecorrido}</p>
              ) : form.kilometrajeRecorrido && helpers?.validateNumeroFormat(form.kilometrajeRecorrido, 0, "Kilometraje") ? (
                <p className="mt-1 text-[11px] text-[#BA7517]">{helpers.validateNumeroFormat(form.kilometrajeRecorrido, 0, "Kilometraje")}</p>
              ) : (
                <p className="mt-1 text-[11px] text-gray-400 dark:text-white/30">Kilómetros del recorrido</p>
              )}
            </div>
            ...
            <div>
              ...
              />
              {errors.rendimiento && <p className="mt-1 text-[11px] text-[#CC1A2E]">{errors.rendimiento}</p>}
              <p className="mt-1 text-[11px] text-gray-400 dark:text-white/30">Dejar vacío para cálculo automático</p>
            </div>
            ...
            <div>
              ...
              />
              {errors.observaciones && <p className="mt-1 text-[11px] text-[#CC1A2E]">{errors.observaciones}</p>}
            </div>

            {/* Preview de cálculo de rendimiento */}
            {form.consumoReal && form.kilometrajeRecorrido && !form.rendimiento && (
              <div className="rounded-lg border border-[#C0DD97] bg-[#EAF3DE] px-3 py-2 dark:border-[#3B6D11]/40 dark:bg-[#3B6D11]/10">
                <p className="text-[11px] font-semibold text-[#3B6D11] dark:text-[#8BC34A]">
                  Rendimiento estimado: {(Number(form.kilometrajeRecorrido) / Number(form.consumoReal)).toFixed(2)} km/L
                </p>
              </div>
            )}
          </div>
