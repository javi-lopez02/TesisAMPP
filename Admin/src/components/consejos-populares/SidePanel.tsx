// src/components/consejos-populares/SidePanel.tsx
import {
  Check, Hash, Loader2, Pencil, Plus,
  Tag, ToggleLeft, ToggleRight, X, UserCircle2,
} from "lucide-react";
import type { FormState } from "../../pages/ConsejosPopularesPage";
...
const generateCodigo = (nombre: string): string => {
  ...
  const prefix = words.map((w) => w.slice(0, 3)).join("").slice(0, 6);
  const num    = String(Math.floor(Math.random() * 900) + 100);
  return `CP-${prefix}-${num}`;
};

// Clases de input según estado de error
const inputClass = (hasError: boolean) =>
  `w-full rounded-lg border px-3.5 py-2.5 text-[13px] text-[#0e1f4d] outline-none transition
   placeholder:text-gray-300
   dark:bg-white/[0.03] dark:text-white dark:placeholder:text-white/20
   ${hasError
...
export const SidePanel = ({
  mode, form, presidentes, errors, loading,
      ...
      <div className="flex flex-1 flex-col gap-4 overflow-y-auto p-5">
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              onChange({ nombre, codigo: nombre ? generateCodigo(nombre) : "" });
            }}
        ...
        <div>
          ...
          {errors.codigo
            ...
            : <p className="mt-1 text-[11px] text-gray-400 dark:text-white/30">Generado automáticamente, puedes editarlo</p>
          }
        </div>
        ...
        <div>
          ...
          {errors.presidenteId
            ? <p className="mt-1 text-[11px] text-[#CC1A2E]">{errors.presidenteId}</p>
        ...
        <div>
          ...
          >
            <span className={`text-[13px] font-semibold ${
              form.activo ? "text-[#3B6D11]" : "text-gray-400 dark:text-white/40"
            }`}>
