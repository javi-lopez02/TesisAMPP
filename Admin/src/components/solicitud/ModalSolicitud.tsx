...
export const ModalSolicitud = ({
  ...
  // 🔹 Handlers directos para Aprobar/Rechazar
  const handleAprobar = async () => {
    await onAprobar(solicitud.id, observaciones.trim());
    onClose();
  };

  const handleRechazar = async () => {
    if (!observaciones.trim()) return; // Validación: motivo obligatorio
    await onRechazar(solicitud.id, observaciones.trim());
    onClose();
  };
                ...
                <div className="rounded-lg border border-[#BA7517]/30 bg-[#F5E6C8] p-4 dark:border-[#BA7517]/40 dark:bg-[#BA7517]/10">
                  ...
                  <div className="flex flex-col gap-2">
                    <button
                      ...
                      disabled={loading}
                      className="flex items-center justify-center gap-2 rounded-lg bg-[#3B6D11] px-4 py-2.5 text-[13px] font-bold text-white transition hover:bg-[#2d5509] disabled:opacity-70"
                    >
                      {loading ? (
                        <>
